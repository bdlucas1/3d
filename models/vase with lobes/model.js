...
thickness = ui.slider({name: 'thickness', min: 0.01, max: 0.05, step: 0.005, value: 0.02})

// const f = (s) => lib.wave(0.0, 0.9, 0.25, .4)(s) - 0.1 * s
diameter = ui.slider({name: 'diameter', min: 0.05, max: 1.5, step: 0.01, value: 0.6})
pts = [{x: 0, y: 1}]
profile = ui.curve({name: 'profile', kind: 'poly', pts: pts})

nLobes = ui.slider({name: 'n lobes', min: 1, max: 64, value: 1})
lobeHeight = ui.slider({name: 'lobe height', min: 0, max: 0.1, step: 0.001, value: 0.1})
lobeHorProfile = ui.curve({name: 'lobe h profile', kind: 'poly', pts: [{x:0, y:1}]})
...
path = lib.line(lib.vec3(0, 0, 0), lib.vec3(0, 0, 1))
height = path(1).minus(path(0)).length()
center = path(1).plus(path(0)).times(0.5)
